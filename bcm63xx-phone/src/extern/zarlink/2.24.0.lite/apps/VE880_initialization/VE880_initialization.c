/** \file VE880_initialization.c
 * VE880_initialization.c
 *
 * This file is a Quick-Start application for a VE880 ABS or Tracker device.
 *
...
#define SUPPLY_CONFIGURATION FLY_BACK_SUPPLY
    ...
    #define TERM_TYPE   VP_TERM_FXS_LOW_PWR

#elif (SUPPLY_CONFIGURATION == INVERTING_BOOST_SUPPLY) 
    #include "VE880_IB_profiles.h"
    #define TERM_TYPE   VP_TERM_FXS_LOW_PWR
...
Vp880LineObjectType lineObj[NB_LINE];
VpStatusType status;
...
int main(void)
{
    ...
    status = VpMakeDeviceObject(VP_DEV_880_SERIES, deviceId, &devCtx, &devObj);
    ...
    for (; lineNum < NB_LINE; lineNum++) {
        ...
        status = VpMakeLineObject(TERM_TYPE, lineNum, &lineCtx[lineNum], &lineObj[lineNum], &devCtx);
        if (status != VP_STATUS_SUCCESS) {
...
#elif (SUPPLY_CONFIGURATION == ABS_SUPPLY)
    status = VpInitDevice(&devCtx, ABS_DEV_PROFILE, ABS_AC_600_FXS, ABS_DC_22MA_CC, ABS_RING_25HZ_SINE, VP_PTABLE_NULL, VP_PTABLE_NULL);
#else 
    #error "SUPPLY_CONFIGURATION is incorrectly set"
#endif
