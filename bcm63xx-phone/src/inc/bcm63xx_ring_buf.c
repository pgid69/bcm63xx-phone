...
int bcm_ring_buf_add_from_user(bcm_ring_buf_t *rb, const __u8 *src, size_t len)
{
   int ret;
   size_t to;
   size_t initial_size = rb->desc.size;

   dd_bcm_pr_debug("bcm_ring_buf_add_from_user(len=%lu)\n", (unsigned long)(len));
...
{
   dd_bcm_pr_debug("bcm_ring_buf_remove(len=%lu)\n", (unsigned long)(len));
...
void bcm_ring_buf_peek(bcm_ring_buf_t *rb, __u8 *dst, size_t len)
{
   size_t offset = rb->desc.off;

   dd_bcm_pr_debug("bcm_ring_buf_peek(len=%lu)\n", (unsigned long)(len));
...
int bcm_ring_buf_remove_to_user(bcm_ring_buf_t *rb, __u8 *dst, size_t len)
{
   ...
   dd_bcm_pr_debug("bcm_ring_buf_remove_to_user(len=%lu)\n", (unsigned long)(len));
